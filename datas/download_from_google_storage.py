...
def download_url(src, target):
  ...
  if src[:4] == 'http':
    ...
    print "download_url src:" + src + ", target:" + target
...
def _downloader_worker_thread(thread_num, q, force, base_url,
  ...
  while True:
    ...
    download_url2(file_url, output_filename) #add by zhw
...
def main(args):
  ...
  if options.no_auth:
    ...
    print 'options.boto:' + options.boto
  ...
  #base_url = 'gs://%s' % options.bucket  zhw
  base_url = 'https://storage.googleapis.com/%s' % options.bucket
